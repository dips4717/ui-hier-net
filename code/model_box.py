...
class GNNChildDecoder(nn.Module):
    ...
    def forward(self, parent_feature):
        ...
        for i in range(self.num_iterations):
            if num_edges > 0:
                node_edge_feats = torch.cat([    # shape [1,1,772] 772 = 256+ 256 +256+ 4
...
class RecursiveDecoder(nn.Module):
    ...
    def node_recon_loss(self, node_latent, gt_node):
        ...
        if gt_node.is_leaf:
            ...
            child_pred_boxes = self.box_decoder(child_feats.view(-1, feature_len))  # [10,10]
            num_child_parts = child_pred_boxes.size(0)

            # perform hungarian matching between pred boxes and gt boxes
            with torch.no_grad():
                child_gt_boxes = torch.cat([child_node.get_box_quat().view(1, -1) for child_node in gt_node.children], dim=0) # [3,10]
