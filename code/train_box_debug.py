...
def train(conf):
    ...
    if not conf.no_console_log:
        header = '     Time    Epoch     Dataset    Iteration    Progress(%)       LR       BoxLoss   LeafLoss      NodeExists      SemLoss   EdgeExists  KLDivLoss   SymLoss    AdjLoss  AnchorLoss  TotalLoss'
...
def forward(batch, data_features, encoder, decoder, device, conf,
    ...
    for obj in objects:
        ...
        # print(f'Time elapsed for 1 object encoding = {toc_obj_enc:.2f} sec')
        ...
        # print(f'Time elapsed for 1 object enc-dec = {toc_obj_encdec:.2f} sec')
...
if __name__ == '__main__':
    ...
    config.val_dataset = 'val_no_other_less_than_10_parts.txt' 
    config.epochs = 200 
    config.model_version = 'model_box' 
