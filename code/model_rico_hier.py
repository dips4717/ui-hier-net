...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan 13 12:42:23 2021
    Modified from model_box.py of structurenet
@author: dipu
"""
"""
...
class BoxEncoder(nn.Module):
    ...
    def __init__(self, feature_size, box_dim=10):
        super(BoxEncoder, self).__init__()
        self.encoder = nn.Linear(box_dim, feature_size)
...
class RecursiveEncoder(nn.Module):
    ...
    def encode_node(self, node):
        if node.is_leaf:
            ...
            return self.box_encoder(node.get_center_scale().view(1,-1))
        else:
...
class BoxDecoder2D(nn.Module):

    def __init__(self, feature_size, hidden_size):
        super(BoxDecoder2D, self).__init__()
        self.mlp = nn.Linear(feature_size, hidden_size)
        self.center = nn.Linear(hidden_size, 2)
    ...
    def forward(self, parent_feature):
        ...
        center = torch.tanh(self.center(feat))
        size = torch.sigmoid(self.size(feat)) * 2
        vector = torch.cat([center, size], dim=1)
        return vector
...
class GNNChildDecoder(nn.Module):
    ...
    def forward(self, parent_feature):
        ...
        """ For Rico Hier """
        # Create edges between children that exist
        # ToDo: Remove the nested for loop. Use edge_indices.expand(1,n) then remove the self connections/
        child_exists_indices = torch.nonzero(child_exists_logits>0)
        child_exists_indices = child_exists_indices[:,1]        
        device = child_exists_indices.device
        edge_indices = []
        device = child_exists_indices.device
        for ii in child_exists_indices:
            for jj in child_exists_indices:
                if ii==jj:
                    continue
        ...
        # ToDo: Explore other edge/graph fomations between the UI components.
        """
        # edge features   # pairwise contatenation of child_feats to predict the existence of edges
        
        edge_latents = torch.cat([
            child_feats.view(batch_size, self.max_child_num, 1, feat_size).expand(-1, -1, self.max_child_num, -1),    #[1, 10, 10, 256]
            child_feats.view(batch_size, 1, self.max_child_num, feat_size).expand(-1, self.max_child_num, -1, -1)     #[1, 10, 10, 256] 
            ], dim=3)
        edge_latents = torch.relu(self.mlp_edge_latent(edge_latents))   #[1, 10, 10, 256]
        ...
        edge_exists_logits = torch.cat(edge_exists_logits_per_type, dim=3)   #torch.Size([1, 10, 10, 4])
        """
        ...
        edge_indices = torch.nonzero(edge_exists_logits > 0)  #torch.Size([163, 4])
        edge_types = edge_indices[:, 3]  # 163
        edge_indices = edge_indices[:, 1:3]    # 163x2
        nodes_exist_mask = (child_exists_logits[0, edge_indices[:, 0], 0] > 0) \
                & (child_exists_logits[0, edge_indices[:, 1], 0] > 0)   # 163
        edge_indices = edge_indices[nodes_exist_mask, :]   # torch.Size([1, 2])
        edge_types = edge_types[nodes_exist_mask]  #1

        # get latent features for the edges
        edge_feats_mp = edge_latents[0:1, edge_indices[:, 0], edge_indices[:, 1], :]  # 1x1x256
        ...
        edge_type_logit = edge_exists_logits[0:1, edge_indices[:, 0], edge_indices[:, 1], :]  #torch.Size([1, 1, 4])
        ...
        edge_type_logit[0:1, range(edge_type_logit.shape[1]), edge_types] = \
                edge_exists_logits[0:1, edge_indices[:, 0], edge_indices[:, 1], edge_types]  # torch.Size([1, 1, 4])
        edge_feats_mp = torch.cat([edge_feats_mp, edge_type_logit], dim=2)  # torch.Size([1, 1, 260])
        """
        ...
        for i in range(self.num_iterations):
            if num_edges > 0:
                node_edge_feats = torch.cat([    # shape [1,1,772] 772 = 256+ 256 +256+ 4 for structure; shape [1,1,512] for RICO; 512=256+256 
                    child_feats[0:1, edge_indices[:, 0], :], # start node features
                    child_feats[0:1, edge_indices[:, 1], :], # end node features
                    ], dim=2) # edge features

                node_edge_feats = node_edge_feats.view(num_edges, -1)
                node_edge_feats = torch.relu(self.node_edge_op[i](node_edge_feats))  #[1,256]
        ...
        child_feats = child_feats.view(-1, self.hidden_size*(self.num_iterations+1))    # torch.Size([10, 768])
        child_feats = torch.relu(self.mlp_child(child_feats))                           # torch.Size([10, 256])
        child_feats = child_feats.view(batch_size, self.max_child_num, self.hidden_size) # torch.Size([1, 10, 256])
...
class RecursiveDecoder(nn.Module):
    ...
    # RICO Boxlosses
    def box_loss(self, *args, **kwargs):
        config = self.conf
        if config.loss_for_box == 'anchor':
            return self.box_anchor_loss(*args, **kwargs)
        elif config.loss_for_box == 'L2':
            return self.box_L2_loss(*args, **kwargs)
        else:
            raise ValueError(f'Unknown loss type: {config.loss}')


    def box_anchor_loss(self, pred_boxes, gt_boxes, reduction='mean'):
        # [N, 4] => [N, K, 2]
        anchors_pred = self.A.center_scale_to_anchors(pred_boxes)
        anchors_gt = self.A.center_scale_to_anchors(gt_boxes)
        loss = F.mse_loss(anchors_pred, anchors_gt, reduction=reduction)
        if reduction == 'none':
            loss = loss.sum(dim=1)
        return loss


    def box_L2_loss(self, pred_boxes, gt_boxes, reduction='mean'):
    ...
    def decode_node(self, node_latent, max_depth, full_label, is_leaf=False):
        ...
        if is_leaf:
            ...
            node = Node(label=full_label, children=child_nodes)
    ...
    def node_recon_loss(self, node_latent, gt_node):
        # self.node_recon_count+=1
        
        if gt_node.is_leaf:
            box = self.box_decoder(node_latent)
            #box_loss = self.boxLossEstimator(box, gt_node.get_box_quat().view(1, -1))
            box_loss = self.box_loss(box, gt_node.get_center_scale().view(1, -1))
            # To add
            ...
            #        self.child_decoder(node_latent)   #[1, 10, 256],  [1, 10, 58], [1, 10, 1], [1, 10, 10, 4]
            ...
            child_pred_boxes = self.box_decoder(child_feats.view(-1, feature_len))  # [4,10]
            num_child_parts = child_pred_boxes.size(0)

            # perform hungarian matching between pred boxes and gt boxes
            with torch.no_grad():
                child_gt_boxes = torch.cat([child_node.get_center_scale().view(1, -1) for child_node in gt_node.children], dim=0) # [3,10]
                ...
                #dist_mat = self.boxLossEstimator(child_gt_boxes_tiled.view(-1, 10), child_pred_boxes_tiled.view(-1, 10)).view(-1, num_gt, num_child_parts) # [1, 3, 10]
                #dist_mat = self.box_loss(child_gt_boxes_tiled.view(-1, 10), child_pred_boxes_tiled.view(-1, 10)).view(-1, num_gt, num_child_parts) # [1, 3, 10]
                dist_mat = self.box_loss(child_gt_boxes_tiled.view(-1, 4), child_pred_boxes_tiled.view(-1, 4), reduction = 'none' )
                dist_mat = dist_mat.mean(dim=1)
                dist_mat = dist_mat.view(-1, num_gt, num_child_parts) # [1, 3, 10]
                ...
                #     edge_types=self.conf.edge_types, device=child_feats.device, type_onehot=False)   #[1, 6],  [1, 6, 2]


                # gt2pred = {gt_idx: pred_idx for gt_idx, pred_idx in zip(matched_gt_idx, matched_pred_idx)}  # dict  e.g. {0: 6, 1: 7, 2: 4}
                # edge_exists_gt = torch.zeros_like(edge_exists_logits)   #[1, 10, 10, 4]
                ...
                #     edge_exists_gt[:, edge_to_idx, edge_from_idx, edge_type_list_gt[0:1, i]] = 1  #[1, 10, 10, 4]
            ...
            box = self.box_decoder(node_latent)    # [1, 10]
            ...
            for i in range(len(matched_gt_idx)):
                ...
                child_box_gt.append(gt_node.children[matched_gt_idx[i]].get_center_scale().view(1, -1))
            ...
            child_sem_pred_logits = torch.cat(child_sem_pred_logits, dim=0)  # [3,58]
            ...
            # call children + aggregate losses   # Recursion 
            pred2allboxes = dict() 
            pred2allleafboxes = dict()
            for i in range(len(matched_gt_idx)):
